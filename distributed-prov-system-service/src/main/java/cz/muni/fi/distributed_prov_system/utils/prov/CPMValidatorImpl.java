...
public class CPMValidatorImpl implements CPMValidator {
    ...
    private static final Set<String> BACKWARD_MANDATORY_ATTRIBUTES = Set.of(
            ...
            ProvConstants.CPM_REFERENCED_BUNDLE_SPEC_V,
            ProvConstants.CPM_REFERENCED_META_BUNDLE_SPEC_V,
            ProvConstants.CPM_REFERENCED_BUNDLE_HASH_VALUE,
            ProvConstants.CPM_HASH_ALG
    );

    private static final Set<String> SPECIALIZED_FORWARD_MANDATORY_ATTRIBUTES = Set.of(
            ProvConstants.CPM_REFERENCED_BUNDLE_ID,
            ProvConstants.CPM_REFERENCED_META_BUNDLE_ID,
            ProvConstants.CPM_REFERENCED_BUNDLE_SPEC_V,
            ProvConstants.CPM_REFERENCED_META_BUNDLE_SPEC_V,
            ProvConstants.CPM_PROVENANCE_SERVICE_URI,
            ProvConstants.CPM_HASH_ALG,
            ProvConstants.CPM_REFERENCED_BUNDLE_HASH_VALUE
    );
    ...
    @Override
    public boolean checkSpecializedForwardConnectorsAttributes(List<Entity> connectors) {
        for (Entity entity : connectors) {
            Set<String> attributeIds = getOtherAttributeNames(entity);
            for (String attribute : SPECIALIZED_FORWARD_MANDATORY_ATTRIBUTES) {
                if (!attributeIds.contains(attribute)) {
                    return false;
                }
            }
    ...
    @Override
    public ValidationResult checkCurrentAgentPresent(Bundle bundle) {
        for (Statement statement : bundle.getStatement()) {
            if (statement instanceof Agent agent && agent.getType() != null) {
                for (Type t : agent.getType()) {
                    Object value = t.getValue();
                    if (value instanceof QualifiedName qn && matchesQualifiedName(qn, ProvConstants.CPM_CURRENT_AGENT)) {
                        return new ValidationResult(true, "ok");
                    } else if (value != null && ProvConstants.CPM_CURRENT_AGENT.equals(value.toString())) {
                        return new ValidationResult(true, "ok");
                    }
                }
            }
        }
        return new ValidationResult(false, "No current agent (cpm:currentAgent) found in bundle.");
    }
    ...
    public ValidationResult checkCpmConstraints(Bundle bundle,
        ...
        for (Entity connector : specializedForwardConnectors) {
            String connectorId = qnameKey(connector.getId());
            ...
            if (connectorSpecializations.size() != 1 ||
                ...
                return new ValidationResult(false,
                        "Specialized forward connector [" + connectorId + "] must specialize exactly one general forward connector.");
            }
        }
        ...
        for (Entity connector : specializedForwardConnectors) {
            if (!checkConnectorDoesNotReferenceSelf(connector, bundleId, metaBundleId)) {
                return new ValidationResult(false, "Specialized forward connector references this bundle or meta bundle.");
            }
        }
        for (Entity connector : backwardConnectors) {
            if (!checkConnectorDoesNotReferenceSelf(connector, bundleId, metaBundleId)) {
